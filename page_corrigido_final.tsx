    ...
    <div className="flex flex-col h-full gap-6">
      ...
      <div className="flex gap-2">
        {["Todos", "Ricardo", "Felipe", "André"].map((barber) => (
          <button
            ...
            className={clsx("px-3 py-1 rounded", activeBarber === barber && "bg-primary text-white")}
          >
            {barber}
          </button>
        ))}
      ...
      <div className="flex flex-col">
        {hours.map((hour) => {
          ...
          const eventsInHour = filteredAppointments.filter(
            (app) => Math.floor(app.hour) === hour
          );

          return (
            <div key={hour} className="flex border-b border-white/10 py-2">
              <div className="w-16 text-right pr-2 text-sm text-white/50">
                {formattedHour}
              </div>
              ...
              <div className="flex-1 flex flex-col gap-2">
                {eventsInHour.map((app) => (
                  <div
                    key={app.id}
                    className={clsx("p-2 rounded border", statusColors[app.status])}
                  >
